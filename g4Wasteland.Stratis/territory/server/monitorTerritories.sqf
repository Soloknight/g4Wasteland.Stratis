...
{
	diag_log "[INFO] monitorTerritories _territorySavingOn is TRUE ... tSave/tLoad invoke";
	...
	_newTerritoryOwners = [];
	{
		...
		if (_currentTerritoryOwner != sideUnknown) then {
			...
			{
				_newTerritoryOwners pushBack [_currentTerritoryName, _x select 7];  // territory/group
			} else {
	...
	diag_log text "[INFO] monitorTerritories: A3W_currentTerritoryOnwers was initialized:";
	{
		diag_log format ["    %1                  %2",_x select 0, _x select 1];   
	} forEach A3W_currentTerritoryOwners;
} else {
	diag_log text "[INFO] monitorTerritories _territorySavingOn is FALSE ... populating currentTerritoryDetails with default data";
	{
...
A3W_territoriesReady = true;  // flag for other processes to proceed
		...
		if (count _newTerritoryData > 0 && {count _newTerritoryDetails > 0}) then
		{
			...
			if (_newContestedStatus || !(_currentTerritoryOwner isEqualTo _newDominantTeam) || (!(_newDominantTeam in [OPFOR,BLUFOR]) && !(_currentTerritoryOwnerGroup isEqualTo _newDominantGroup))) then
			{
				...
				if ((_newCapPointTimer >= _capturePeriod) && _turnOver) then
				{
					...
					diag_log format ["[INFO] monitorTerritories._handleCapPointTick: '%1' was captured by '%2'!", _currentTerritoryName, _newDominantTeam];
