...
if (MF_ITEMS_CAMO_NET1 call mf_inventory_count <= 0) exitWith {
...
_hasFailed = {
	...
	switch (true) do {
		...
		case (MF_ITEMS_CAMO_NET1 call mf_inventory_count <= 0): {_text = ERR_NO_NETS};
...
if (_success) then {
	...
	_netting = createVehicle [MF_ITEMS_CAMO_NET_TYPE1, _pos, "CAN COLLIDE"];
	_netting allowdamage false;
