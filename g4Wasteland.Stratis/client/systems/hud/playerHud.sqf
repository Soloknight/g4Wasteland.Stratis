...
while {true} do
{
	...
	{
		_tempString = format ["<t color='#A0FFFFFF'>g4 Wasteland %1<br/>www.a3wasteland.com</t>", getText (configFile >> "CfgWorlds" >> worldName >> "description")];
		_yOffset = 0.28;
			...
			else
			{
				...
				if (_globalVoiceTimer >= _globalVoiceWarnTimer) then
				{
					...
					if (_globalVoiceWarning < _globalVoiceMaxWarns) then
					{
						...
						_msgTitle spawn
						{
							_this hintC parseText "You will be kicked/banned for using VON in GLOBAL/SIDE channels. Download TeamSpeak and join TS.G4-GAMERS.COM. <br/><br/> <t color='#ff0000'>How to unbind Push to Talk and default to Group:</t><br/> <img size='30'  image='client\images\information\von.jpg'/>"; //FROM TOP SERVER
						};						
					}
					else
					{
